...
def _all_papers(include_archive: bool = True) -> list[dict[str, Any]]:
    ...
    # Process newest first, then keep first occurrence of each arXiv ID.
...
def _paper_text(item: dict[str, Any]) -> str:
    ...
    fields = [
        ...
        paper.get("comment"), authors_text,
        analysis.get("problem_statement"), analysis.get("bottleneck_addressed"),
        analysis.get("executive_summary"), tags_text,
        metrics.get("primary_metric"), metrics.get("baseline"),
        tech.get("method"), tech.get("architecture"),
        tech.get("implementation_details"),
        conf.get("conference"), conf.get("year"), conf.get("category"), conf.get("rank"),
    ]
...
def _score(item: dict[str, Any], query: str) -> float:
    ...
    for term in terms:
        ...
        if term in tags:
            score += 4.0
        if term in conference:
            score += 3.0
        score += min(haystack.count(term), 4) * 1.0

    # Reward results that match more distinct query terms.
...
def _compact(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "field": conf.get("category"),
        "rank": conf.get("rank"),
        "problem_statement": analysis.get("problem_statement"),
        "executive_summary": analysis.get("executive_summary"),
        "relevance_tags": analysis.get("relevance_tags", []),
...
def search(query: str, limit: int = 10, include_archive: bool = True) -> dict[str, Any]:
    ...
    Searches titles, abstracts, authors, conference metadata, problem statements,
...
def fetch(id: str) -> dict[str, Any]:
    """Fetch the complete analyzed record for one paper by arXiv ID.

    The ID may include or omit the arXiv version suffix, e.g. 2608.09931v1 or
    ...
    for item in _all_papers(include_archive=True):
        ...
        candidate_base = re.sub(r"v\d+$", "", candidate)
        if candidate == wanted or candidate_base == wanted_base:
            result = dict(item)
...
def recent_papers(
    ...
    def matches(item: dict[str, Any]) -> bool:
        conf = item.get("conference_info", {}) or {}
        if date and item.get("source_date") != date:
            return False
        if conference and conference.lower() not in str(conf.get("conference") or "").lower():
            return False
        if field and field.lower() not in str(conf.get("category") or "").lower():
            return False
        if rank and rank.lower() != str(conf.get("rank") or "").lower():
            return False
        return True
    ...
    return {
        ...
        "papers": [_compact(item) for item in filtered[:limit]],
    }
...
def paper_stats(date: str | None = None) -> dict[str, Any]:
    ...
    conferences: Counter[str] = Counter()
    fields: Counter[str] = Counter()
    ranks: Counter[str] = Counter()
    arxiv_categories: Counter[str] = Counter()

    for item in items:
        paper = item.get("paper", {}) or {}
        conf = item.get("conference_info", {}) or {}
        if conf.get("conference"):
            conferences[str(conf["conference"])] += 1
        if conf.get("category"):
            fields[str(conf["category"])] += 1
        if conf.get("rank"):
            ranks[str(conf["rank"])] += 1
        if paper.get("categories"):
            arxiv_categories[str(paper["categories"])] += 1

    return {
        "date": date or "rolling_archive",
        "total_unique_papers": len(items),
        "by_conference": dict(conferences.most_common()),
        "by_field": dict(fields.most_common()),
        "by_rank": dict(ranks.most_common()),
        "by_arxiv_category": dict(arxiv_categories.most_common()),
    }


def _allowed_hosts() -> list[str]:
    """Build the MCP Host allowlist for local, Render, and the public custom domain."""
    hosts = {
        "localhost",
        "localhost:*",
        "127.0.0.1",
        ...
        "[::1]",
        "[::1]:*",
        # Public custom domain used by Claude/ChatGPT/Mistral.
        "arxiv.sahilpardasani.com",
        "arxiv.sahilpardasani.com:*",
    }
...
def _allowed_origins() -> list[str]:
    origins = {"https://arxiv.sahilpardasani.com"}
    render_host = os.getenv("RENDER_EXTERNAL_HOSTNAME", "").strip()
    if render_host:
...
# streamable_http_path="/" means the parent FastAPI mount at /mcp is the public
# endpoint itself: https://<host>/mcp (not /mcp/mcp).
